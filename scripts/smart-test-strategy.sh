...
HIGH_RISK_FILES=$(git diff --name-only HEAD~1..HEAD 2>/dev/null | grep -E "routes/|controllers/|middleware/|auth|security|database" || true)
...
if [[ $RISK_SCORE -ge 7 ]]; then
  ...
  # Runs: npm run test:comprehensive 2>/dev/null || npm run test:integration 2>/dev/null || npm test
